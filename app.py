...
# ── Restriction réseau privé uniquement ───────────────────────────────────────
@app.before_request
def _local_only():
    """Refuse les requêtes hors réseau local sur les routes API."""
    if request.path.startswith("/api/"):
        remote = request.remote_addr
        try:
            remote_ip = ipaddress.ip_address(remote)
            allowed = remote_ip.is_loopback or remote_ip.is_private
        except ValueError:
            allowed = remote == "localhost"
        if not allowed:
            return jsonify({"error": "Accès refusé — réseau local uniquement"}), 403
...
def _row(label: str, value: str, bg: str = _WHITE, text_color: str = _TEXT_DARK) -> str:
    ...
    return (
        ...
        f'<td style="background-color:{bg};color:{text_color};font-weight:bold;padding:14px 16px;'
        f'text-align:center;vertical-align:middle;border:{_BORDER};">{safe}</td>'
        f'</tr>'
    )


def _title_row(title_text: str, colspan: int = 2) -> str:
    return (
        f'<tr><td colspan="{colspan}" style="background-color:{_LABEL_BG};'
        f'font-weight:bold;font-size:18px;text-align:center;padding:16px;'
        f'border:{_BORDER};">{_html.escape(title_text.upper())}</td></tr>'
    )


def _spacer_row(colspan: int = 2, height: int = 22) -> str:
    return (
...
def _sig_html() -> str:
    ...
    return (
        "<br><br>"
        f'<p style="font-family:{_FONT};font-size:13px;color:#333333;margin:0;">'
        f'Cordialement,<br>'
