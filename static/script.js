...
switchSection = function(section) {
  ['saisie','admin','rapports'].forEach(s => {
    const tab = document.getElementById(`stab-${s}`);
    if (tab) tab.classList.toggle('active', s === section);
    const el = document.getElementById(`section-${s}`);
    if (el) el.style.display = s === section ? '' : 'none';
  ...
  if (section === 'rapports') { rapCharger(); return; }
...
function srRafraichirApercu() {
  ...
  srTimerApercu = setTimeout(() => {
    srDemanderApercu();
    if (typeof srChargerSorties === 'function') srChargerSorties();
  }, 220);
}
...
async function srEnregistrer() {
  ...
  try {
    ...
    // Rattacher les captures collées avant que l'incident n'existe
    if (SR.preuves.length) {
      await fetch(`/api/v5/incidents/${data.id}/preuves/rattacher`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fichiers: SR.preuves })
      });
    }
    showToast(`Incident ${SR.ticket} enregistré`
              + (SR.preuves.length ? ` · ${SR.preuves.length} preuve(s)` : ''), 'success');
  } catch (e) {
    console.error(e);
    showToast('Erreur réseau', 'error');
  }
...
async function srCopier(brut) {
  ...
  try {
    if (brut || !navigator.clipboard.write) {
      await navigator.clipboard.writeText(texte);
    } else {
      await navigator.clipboard.write([new ClipboardItem({
        'text/html': new Blob([SR.dernierApercu.corps_html], { type: 'text/html' }),
...
function srVider() {
  ...
  SR.valeurs = {}; SR.incidentId = null; SR.preuves = [];
  if (typeof srRendrePreuves === 'function') srRendrePreuves();
...
// ════════════════════════════════════════════════════════════════════════
// RAPPORTS — fin de shift, hebdomadaire, mensuel
// ════════════════════════════════════════════════════════════════════════
//
// Le rendu vient du serveur (comme l'aperçu des communications) et s'affiche
// dans une iframe : le rapport garde sa propre mise en forme, imprimable,
// indépendante du thème de l'application.

const RAP = { type: 'shift', dernier: null };

async function rapCharger() {
  const cadre = document.getElementById('rap-rendu');
  const zone = document.getElementById('rap-indicateurs');
  if (!cadre) return;

  const dateRef = document.getElementById('rap-date')?.value || '';
  const suffixe = dateRef ? '&date=' + encodeURIComponent(dateRef) : '';

  zone.innerHTML = '<div class="rap-attente">Génération…</div>';

  try {
    // Indicateurs (JSON) et rendu (HTML) en parallèle
    const [resJson, resHtml] = await Promise.all([
      fetch(`/api/v5/rapports/${RAP.type}?format=json${suffixe}`),
      fetch(`/api/v5/rapports/${RAP.type}?format=html${suffixe}`)
    ]);
    const donnees = await resJson.json();
    RAP.dernier = donnees;

    if (!donnees.ok) {
      zone.innerHTML = `<div class="rap-attente">${escHtml(donnees.error || 'Erreur')}</div>`;
      return;
    }

    const cartes = [['Total', donnees.total]];
    [['en_cours', 'En cours'], ['clos', 'Clôturés'],
     ['ouverts_fin_periode', 'Ouverts'], ['majeurs', 'Majeurs'],
     ['nb_majeurs', 'Majeurs'], ['regularisations', 'Régularisations']
    ].forEach(([k, l]) => { if (k in donnees) cartes.push([l, donnees[k]]); });
    if (donnees.duree_moyenne != null) {
      const m = donnees.duree_moyenne;
      cartes.push(['Durée moyenne',
        String(Math.floor(m / 60)).padStart(2, '0') + 'h' + String(m % 60).padStart(2, '0')]);
    }

    zone.innerHTML = `
      <div class="rap-periode">${escHtml(donnees.titre)} · du ${escHtml(donnees.debut)} au ${escHtml(donnees.fin)}</div>
      <div class="rap-cartes">${cartes.map(([l, v]) =>
        `<div class="rap-carte"><div class="v">${escHtml(String(v))}</div><div class="l">${escHtml(l)}</div></div>`
      ).join('')}</div>`;

    cadre.srcdoc = await resHtml.text();
  } catch (e) {
    console.error(e);
    zone.innerHTML = '<div class="rap-attente">Rapport indisponible</div>';
  }
}
...
function rapExcel() {
  const dateRef = document.getElementById('rap-date')?.value || '';
  const suffixe = dateRef ? '&date=' + encodeURIComponent(dateRef) : '';
  window.location = `/api/v5/rapports/${RAP.type}?format=xlsx${suffixe}`;
}

async function rapEnvoyer() {
  const cadre = document.getElementById('rap-rendu');
  const html = cadre?.srcdoc || '';
  if (!html) return;
  try {
    await navigator.clipboard.write([new ClipboardItem({
      'text/html': new Blob([html], { type: 'text/html' })
    })]);
    showToast('Rapport copié — le coller dans un nouveau message', 'success');
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.rap-onglet').forEach(b => {
    b.onclick = () => {
      document.querySelectorAll('.rap-onglet').forEach(x => x.classList.remove('actif'));
      b.classList.add('actif');
      RAP.type = b.dataset.r;
      rapCharger();
    };
  });
  document.getElementById('rap-btn-actualiser')?.addEventListener('click', rapCharger);
  document.getElementById('rap-btn-imprimer')?.addEventListener('click', rapImprimer);
  document.getElementById('rap-btn-excel')?.addEventListener('click', rapExcel);
  document.getElementById('rap-btn-mail')?.addEventListener('click', rapEnvoyer);
  document.getElementById('rap-date')?.addEventListener('change', rapCharger);
});

// ════════════════════════════════════════════════════════════════════════
// PREUVES — collage de captures d'écran
// ════════════════════════════════════════════════════════════════════════
//
// Le geste visé est celui déjà pratiqué dans Outlook : capturer puis coller
// directement (Ctrl+V). Le dépôt de fichier reste possible, mais le collage
// est le chemin principal.
//
// Les captures sont téléversées immédiatement, avant même que l'incident
// n'existe : on colle la preuve pendant qu'on la regarde. Elles sont
// rattachées à l'incident au moment de l'enregistrement.

SR.preuves = [];

async function srTeleverser(fichier, legende) {
  if (!fichier.type.startsWith('image/')) {
    showToast('Seules les images sont acceptées', 'error');
    return;
  }
  const donnee = await new Promise((ok, ko) => {
  ...
  try {
    const r = await fetch('/api/v5/preuves/televerser', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        donnee,
        legende: legende || '',
        incident_id: SR.incidentId || null
      })
    });
    const d = await r.json();
    if (!d.ok) { showToast(d.error || 'Échec', 'error'); return; }
    SR.preuves.push({ fichier: d.fichier, url: d.url, legende: legende || '', taille: d.taille_ko });
    srRendrePreuves();
    showToast(`Capture ajoutée (${d.taille_ko} Ko)`, 'success');
  } catch (e) {
    showToast('Téléversement impossible', 'error');
  }
}

function srRendrePreuves() {
  const galerie = document.getElementById('sr-galerie');
  const vide = document.getElementById('sr-preuves-vide');
  if (!galerie) return;

  if (vide) vide.style.display = SR.preuves.length ? 'none' : '';

  galerie.innerHTML = SR.preuves.map((p, i) => `
    <figure class="sr-preuve">
      <img src="${escAttr(p.url)}" alt="${escAttr(p.legende || 'Capture ' + (i + 1))}">
      <input class="sr-legende" data-i="${i}" value="${escAttr(p.legende)}"
             placeholder="Légende…" autocomplete="off">
      <button class="sr-preuve-retirer" data-i="${i}" title="Retirer">×</button>
    </figure>`).join('');

  galerie.querySelectorAll('.sr-preuve-retirer').forEach(b => {
    b.onclick = () => { SR.preuves.splice(+b.dataset.i, 1); srRendrePreuves(); };
  });
  galerie.querySelectorAll('.sr-legende').forEach(el => {
    el.oninput = () => { SR.preuves[+el.dataset.i].legende = el.value; };
  });
}
...
// ════════════════════════════════════════════════════════════════════════
//
// Remplacent l'outil autonome Web_SMS_Sup_v2.2.html, qui reconstruisait ces
// informations en analysant le texte d'un e-mail par expressions régulières.
// Tout vient désormais des données structurées.

SR.sortie = 'mail';
SR.dernieresSorties = null;

async function srChargerSorties() {
  try {
    const r = await fetch('/api/v5/sorties', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(srCharge())
    });
    SR.dernieresSorties = await r.json();
    if (SR.sortie !== 'mail') srAfficherSortie();
  } catch (e) {
    console.error('Sorties indisponibles', e);
  }
}
...
function srAfficherSortie() {
  const cadre = document.getElementById('sr-apercu');
  const texte = document.getElementById('sr-texte');
  const enveloppe = document.getElementById('sr-enveloppe');
  const glose = document.getElementById('sr-glose');
  if (!cadre || !texte) return;

  const estMail = SR.sortie === 'mail';
  cadre.style.display = estMail ? '' : 'none';
  texte.style.display = estMail ? 'none' : '';
  if (enveloppe) enveloppe.style.display = estMail ? '' : 'none';
  if (estMail) { srRafraichirApercu(); return; }

  const s = SR.dernieresSorties;
  if (!s || !s.ok) { texte.textContent = 'Génération…'; return; }

  if (SR.sortie === 'sms') {
    const trop = s.sms_longueur > s.sms_max;
    texte.textContent = s.sms;
    if (glose) {
      glose.innerHTML = `<b>${s.sms_longueur}</b> caractères sur ${s.sms_max}` +
        (trop ? ' — <b>message trop long</b>, il sera découpé par l\'opérateur.' : '.');
    }
  } else if (SR.sortie === 'rapport') {
    texte.textContent = s.rapport_court;
    if (glose) glose.textContent = "Bloc de synthèse à coller dans un rapport ou un point d'avancement.";
  } else {
    texte.textContent = s.ligne_colonnes
      .map((c, i) => `${c.padEnd(30, '.')} ${s.ligne_valeurs[i] || ''}`).join('\n');
    if (glose) {
      glose.innerHTML = "Ligne prête à coller dans le classeur de suivi (colonnes séparées par des tabulations). " +
        "<b>Les durées sont calculées</b> — elles restaient vides dans l'ancien outil.";
    }
  }
}

async function srCopierSortie() {
  const s = SR.dernieresSorties;
  if (SR.sortie === 'mail') return srCopier(false);
  if (!s) return;

  const contenu = SR.sortie === 'sms' ? s.sms
                : SR.sortie === 'rapport' ? s.rapport_court
                : s.ligne_suivi;
  try {
    await navigator.clipboard.writeText(contenu);
    showToast(SR.sortie === 'ligne' ? 'Ligne copiée — coller dans Excel' : 'Copié', 'success');
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
  // ── Preuves : collage, dépôt, sélection ──
  const zone = document.getElementById('sr-preuves');
  if (zone) {
    zone.addEventListener('paste', e => {
      ...
      e.preventDefault();
      images.forEach(i => { const f = i.getAsFile(); if (f) srTeleverser(f); });
    });
    ...
    zone.addEventListener('drop', e => {
      ...
      Array.from(e.dataTransfer?.files || []).forEach(f => srTeleverser(f));
    });
  }

  // Coller depuis n'importe où dans la section, si aucun champ n'a le focus
  ...
  // ── Onglets de sortie ──
  document.querySelectorAll('.sr-sortie').forEach(b => {
    b.onclick = () => {
      document.querySelectorAll('.sr-sortie').forEach(x => x.classList.remove('actif'));
      b.classList.add('actif');
      SR.sortie = b.dataset.s;
      srAfficherSortie();
    };
  });

  // Le bouton « Copier » suit l'onglet affiché
  const btnCopier = document.getElementById('sr-btn-copier');
  if (btnCopier) {
    btnCopier.removeEventListener('click', srCopier);
    btnCopier.onclick = srCopierSortie;
  }
});

// ════════════════════════════════════════════════════════════════════════
// THÈMES — Design System SmartSup
// ════════════════════════════════════════════════════════════════════════
//
// PÉRIMÈTRE : le thème n'affecte QUE l'interface de l'application.
// Les e-mails et les rapports sont rendus par le serveur dans des iframes
// isolées : ils conservent leur charte propre quel que soit le thème choisi.
//
// Le thème est appliqué avant le rendu par un script inline dans <head>
// (sinon la page clignoterait au chargement). Ce module gère le sélecteur,
// la persistance et la synchronisation avec le paramétrage serveur.

const THEMES = [
  { code: 'clair',  nom: 'Clair',       detail: 'Lumineux, confortable en journée',   glyphe: '☀' },
  { code: 'sombre', nom: 'Sombre',      detail: 'Confortable en usage prolongé',       glyphe: '◐' },
  { code: 'furtif', nom: 'Furtif',      detail: 'Supervision nocturne, très sobre',    glyphe: '◉' },
  { code: 'noc',    nom: 'NOC',         detail: 'Grand écran, états très lisibles',    glyphe: '▣' },
  { code: 'auto',   nom: 'Automatique', detail: 'Suit le réglage du système',          glyphe: '⚙' }
];

const CLE_THEME = 'smartsup-theme';

function themeActuel() {
  return document.documentElement.getAttribute('data-theme') || 'sombre';
}

function appliquerTheme(code, memoriser = true) {
  const racine = document.documentElement;

  // La transition n'est posée que le temps du basculement : la laisser en
  // permanence ralentirait toutes les interactions courantes.
  racine.classList.add('bascule-theme');
  racine.setAttribute('data-theme', code);

  setTimeout(() => racine.classList.remove('bascule-theme'), 280);

  if (memoriser) {
    try { localStorage.setItem(CLE_THEME, code); } catch (e) { /* mode privé */ }
    // Enregistré aussi côté serveur, pour que le choix survive au navigateur
    fetch('/api/v5/admin/parametres', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 'ui.theme': code })
    }).catch(() => { /* le thème reste appliqué même si l'écriture échoue */ });
  }

  const nom = document.getElementById('theme-nom');
  const t = THEMES.find(x => x.code === code);
  if (nom && t) nom.textContent = t.nom;

  themeRendreMenu();
}

function themeRendreMenu() {
  const menu = document.getElementById('theme-menu');
  if (!menu) return;
  const actuel = themeActuel();

  menu.innerHTML = '<div class="theme-menu-titre">Apparence</div>' +
    THEMES.map(t => `
      <button class="theme-option ${t.code === actuel ? 'actif' : ''}"
              data-theme-code="${escAttr(t.code)}" role="menuitem">
        <span class="theme-option-glyphe">${t.glyphe}</span>
        <span class="theme-option-nom">${escHtml(t.nom)}
          <span class="theme-option-detail">${escHtml(t.detail)}</span>
        </span>
        <span class="theme-apercu">
          <span class="tap-${t.code}-1"></span>
          <span class="tap-${t.code}-2"></span>
          <span class="tap-${t.code}-3"></span>
        </span>
      </button>`).join('');

  menu.querySelectorAll('[data-theme-code]').forEach(b => {
    b.onclick = () => {
      appliquerTheme(b.dataset.themeCode);
      themeFermerMenu();
    };
  });
}

function themeOuvrirMenu() {
  document.getElementById('theme-menu')?.classList.add('ouvert');
  document.getElementById('theme-declencheur')?.setAttribute('aria-expanded', 'true');
}

function themeFermerMenu() {
  document.getElementById('theme-menu')?.classList.remove('ouvert');
  document.getElementById('theme-declencheur')?.setAttribute('aria-expanded', 'false');
}

// ── État d'incident : couleur + forme + libellé ─────────────────────────────
//
// Jamais la couleur seule. Chaque état porte une pastille de forme distincte,
// pour rester lisible en cas de daltonisme comme en mode furtif.
const ETATS = {
  critique: { classe: 'etat-critique', libelle: 'Critique' },
  encours:  { classe: 'etat-encours',  libelle: 'En cours' },
  attente:  { classe: 'etat-attente',  libelle: 'Investigation' },
  resolu:   { classe: 'etat-resolu',   libelle: 'Résolu' },
  neutre:   { classe: 'etat-neutre',   libelle: 'Non avéré' }
};

function badgeEtat(cle, libelle, actif) {
  const e = ETATS[cle] || ETATS.neutre;
  return `<span class="etat ${e.classe}${actif ? ' actif' : ''}">
            <span class="etat-pastille"></span>${escHtml(libelle || e.libelle)}
          </span>`;
}

/** Déduit l'état visuel d'un incident depuis ses données. */
function etatIncident(inc) {
  if (inc.statut_documentaire === 'cloture' || inc.date_fin) return 'resolu';
  if (inc.statut_documentaire === 'regularise') return 'resolu';
  if (inc.priorite === 'P1') return 'critique';
  if (inc.statut_documentaire === 'en_suivi') return 'attente';
  return 'encours';
}

document.addEventListener('DOMContentLoaded', () => {
  themeRendreMenu();
  const t = THEMES.find(x => x.code === themeActuel());
  const nom = document.getElementById('theme-nom');
  if (nom && t) nom.textContent = t.nom;

  const declencheur = document.getElementById('theme-declencheur');
  if (declencheur) {
    declencheur.onclick = e => {
      e.stopPropagation();
      const menu = document.getElementById('theme-menu');
      menu?.classList.contains('ouvert') ? themeFermerMenu() : themeOuvrirMenu();
    };
  }
  document.addEventListener('click', e => {
    if (!e.target.closest('#theme-zone')) themeFermerMenu();
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') themeFermerMenu();
  });

  // Le mode automatique doit réagir si le système change de réglage
  if (window.matchMedia) {
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
      if (themeActuel() === 'auto') appliquerTheme('auto', false);
    });
  }
});
