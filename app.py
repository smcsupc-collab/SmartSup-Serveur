...
# _title_table n'est plus appelee par build_html_body / _ran_nbn / _notification
# (remplacee par _title_row + _spacer_row, fix "tableau unique") ; conservee
# telle quelle au cas ou elle serait utilisee ailleurs.
def _title_row(title: str, colspan: int = 2) -> str:
    return (
        f'<tr><td colspan="{colspan}" style="padding:14px 16px;border:1px solid #000000;'
        f'text-align:center;font-weight:bold;font-family:{_FONT};font-size:16px;'
        f'background-color:{_LABEL_BG};color:{_LABEL_TEXT};letter-spacing:2px;">'
        f'{title.upper()}'
        f'</td></tr>'
...
def _spacer_row(colspan: int = 2, height: int = 14) -> str:
    # Ligne fantome : border:hidden (pas "none"). En border-collapse:collapse,
    # "hidden" l'emporte toujours sur la bordure d'une cellule voisine —
    # "none" pourrait se faire recouvrir par la bordure du titre ou de la 1ere ligne.
    return (
        f'<tr><td colspan="{colspan}" style="border:hidden;padding:0;'
...
def build_html_body(data: dict) -> str:
    ...
    table_html = (
        ...
        + "".join([_title_row(title_text), _spacer_row()] + rows) + "</table>"
    )
    return (
...
def build_html_body_notification(data: dict) -> str:
    ...
    return f'<html><body style="font-family:{_FONT};">' + table_html + sig_html + '</body></html>'
